...
class QuizInterFace:
    ...
    def get_next_question(self):
        if self.quiz.still_has_questions():
            ...
            self.canvas.itemconfig(self.question_text, text=f"You have reached the end of the quiz and your score is: {self.quiz.score}", fill=THEME_COLOR)
            self.canvas.config(bg="white")
    ...
    def give_feedback(self, is_right):
        if is_right:
            self.canvas.config(bg= "#01937C" )
            self.canvas.itemconfig(self.question_text, fill="white")
        else:
            self.canvas.config(bg="#FB3640")
            self.canvas.itemconfig(self.question_text, fill="white")
